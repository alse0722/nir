...
public class GraphParser1_0 {
  ...
  private static int calculateTotalDominatingNumber(List<List<Integer>> graph) {
    int n = graph.size();
    boolean[] dominated = new boolean[n]; // Флаги для отслеживания вершин, покрытых доминирующим множеством
    int domNum = 0; // Число полного доминирования

    for (int v = 0; v < n; v++) {
        if (!dominated[v]) { // Если текущая вершина не покрыта
            domNum++; // Увеличиваем число полного доминирования
            for (int u = 0; u < n; u++) {
                if (graph.get(v).get(u) == 1) { // Если есть ребро между вершинами v и u
                    dominated[u] = true; // Помечаем вершину u как покрытую
                }
            }
        }
    }
