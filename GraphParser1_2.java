...
public class GraphParser1_2 {
  ...
  private static boolean isTotalDominatingSet(List<Integer> subset, List<List<Integer>> graph) {
    ...
    // Проверяем, что все вершины графа доминируемы и вершины из подмножества также имеют соседа
    for (int u : subset) {
      if (!hasNeighbor(u, graph)) {
        return false;
      }
    }
  ...
  // Вспомогательный метод для проверки, имеет ли вершина соседа
  private static boolean hasNeighbor(int vertex, List<List<Integer>> graph) {
    int n = graph.size();
    for (int v = 0; v < n; v++) {
      if (graph.get(vertex).get(v) == 1) {
        return true;
      }
    }
    return false;
