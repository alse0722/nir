...
public class GraphParserGeo0_0 {
  ...
  private static int calculateIndependentGeoNumber(int[][] graph) {
    ...
    // Используем мемоизацию для хранения результатов проверок
    Map<Integer, Boolean> memo = new HashMap<>();

    // Перебор всех подмножеств вершин
    for (int subset = 1; subset < (1 << n); subset++) {
      int setSize = Integer.bitCount(subset);
      if (setSize >= minGeoSetSize) {
        continue;
      }

      if (isIndependentGeoSet(subset, graph, dist, n, memo)) {
        minGeoSetSize = setSize;
      }
    }
