...
public class NIR_v0_2 {
  ...
  public static void main(String[] args) {
    ...
    int graphsToFindv2 = 1;
    int graphsToFindv3 = 1;
    int graphsToFindv4 = 1;
    int graphsToFindv5 = 3;
    int graphsToFindv6 = 3;
    int graphsToFindv7 = 2;
    int graphsToFindv8 = 4;
    int graphsToFindv9 = 14;
    int graphsToFindv10 = 9;
    int graphsToFindv11 = 12;
      ...
      switch (vertexCount) {
        case 2 -> {
          if (graphsToFindv2 == 0)
            ...
            if (domNumber == 2) {
              geoNumber = calculateIndependentGeoNumber(graph);
              if (geoNumber == 0) {
                resultMessage.add(Arrays.deepToString(graph) + " {dom: " + domNumber + ", geo: " + geoNumber + "}\n");
                graphsToFindv2--;
              }
            }
          }
    ...
    // System.out.println(",");
    // }
    // System.out.print("{dom_number: " + entry.getKey().get(0) + ", geo_number: " +
    // entry.getKey().get(1) + ", count: "
    // + entry.getValue() + "}");
    // first = false;
    // }
    ...
    // try {
    // FileWriter writer = new FileWriter("java/NIR_v0.1_{" + vertexCount + "_" +
    // startTime + "}_vertexes_output.txt");
    // writer.write("Elapsed time: " + elapsedTime + " seconds\n");
    // writer.write("Statistics:\n");
    // writer.write("┌─dom_number─┬─geo_number─┬─count─┐\n");
    // for (Map.Entry<List<Integer>, Integer> entry : stats.entrySet()) {
    // writer
    // .write(String.format("│%12d│%12d│%7d│%n", entry.getKey().get(0),
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter("java/v" + vertexCount + "_graphs_" + startTime + ".txt"))) {
      for (String msg : resultMessage) {
        writer.write(msg);
        // writer.newLine(); // Переход на новую строку после каждого сообщения
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
